...
| AccountsService | [accountsservice/accountsservice](https://gitlab.freedesktop.org/accountsservice/accountsservice) | [MR !176](https://gitlab.freedesktop.org/accountsservice/accountsservice/-/merge_requests/176) | discussion | Adds persistent birth-date storage, D-Bus `GetBirthDate`/`SetBirthDate` methods, PolicyKit-gated access, `libaccountsservice` API exposure, and JSON user-record bridging. Operationally, this is an advanced, near-merge risk. | Lowers the barrier for desktop account UIs and other consumers to normalize age-related personal-data handling, turning AccountsService into a standard desktop account-data service. |
